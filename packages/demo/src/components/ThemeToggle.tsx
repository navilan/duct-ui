...
export interface ThemeToggleEvents extends BaseComponentEvents {
  themeChanged: (el: HTMLElement, theme: 'duct' | 'duct-dark') => void
}

export interface ThemeToggleLogic {
  getCurrentTheme: () => 'duct' | 'duct-dark'
  setTheme: (theme: 'duct' | 'duct-dark') => void
  toggleTheme: () => void
}

export interface ThemeToggleProps {
  initialTheme?: 'duct' | 'duct-dark'
  'on:bind'?: (el: HTMLElement) => void
  'on:themeChanged'?: (el: HTMLElement, theme: 'duct' | 'duct-dark') => void
  'on:release'?: (el: HTMLElement) => void
}

function render(props: BaseProps<ThemeToggleProps>) {
  const { initialTheme = 'duct', ...moreProps } = props
...
function bind(el: HTMLElement, eventEmitter: EventEmitter<ThemeToggleEvents>): BindReturn<ThemeToggleLogic> {
  ...
  function getInitialTheme(): 'duct' | 'duct-dark' {
    ...
    if (hasOverride && (stored === 'duct' || stored === 'duct-dark')) {
      return stored
    }
    
    // Check system preference only if no manual override
    const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches
    return prefersDark ? 'duct-dark' : 'duct'
  }

  let currentTheme = getInitialTheme()

  function updateIcons() {
    if (currentTheme === 'duct-dark') {
  ...
  function applyTheme(theme: 'duct' | 'duct-dark') {
    // Force override DaisyUI's automatic theme switching by setting both data-theme and theme
    document.documentElement.setAttribute('data-theme', theme)
    document.documentElement.setAttribute('theme', theme)
    document.body.setAttribute('data-theme', theme)
    
    // Add a custom CSS class to force the theme
    document.documentElement.classList.remove('theme-duct', 'theme-duct-dark')
  ...
  function toggleTheme() {
    const newTheme = currentTheme === 'duct' ? 'duct-dark' : 'duct'
    applyTheme(newTheme)
  }
  ...
  function handleSystemThemeChange(e: MediaQueryListEvent) {
    ...
    const hasOverride = localStorage.getItem('theme-override') === 'true'
    if (!hasOverride) {
      const newTheme = e.matches ? 'duct-dark' : 'duct'
...
const id = { id: "duct-demo/theme-toggle" }
